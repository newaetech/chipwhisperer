...
class SADExplorer(util.DisableNewAttr):
    ...
    def get_legend_items(self, segments):
        ...
        if ttimes:
            items.append('ttimes: %d / %d / %d' % (min(ttimes), max(ttimes), int(np.average(ttimes))))
        else:
            items.append('ttimes: N/A')
    ...
    def update_plot(self, refstart='', samples='', segments='', threshold='', interval_threshold='', exclude='', emode=False, show_diffs=False, show_text_legend=False, show_plot_legend=False):
        ...
        if show_diffs:
            ...
            self.Rp.data_source.data = {'y': self.reftrace[refstart:refstart+samples] + interval_threshold,
                                        'x': range(samples)}
            self.Rm.data_source.data = {'y': self.reftrace[refstart:refstart+samples] - interval_threshold,
                                        'x': range(samples)}
            self.Rf.data_source.data = {'y1': self.reftrace[refstart:refstart+samples] - interval_threshold,
                                        'y2': self.reftrace[refstart:refstart+samples] + interval_threshold,
                                        'x': range(samples)}
