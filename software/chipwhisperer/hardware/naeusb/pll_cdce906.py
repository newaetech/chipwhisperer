...
class PLLCDCE906(object):
    ...
    def outputUpdateOutputs(self, outnum):
        ...
        if outnum == 0:
            outpin = 0
            src = self.parent.findParam('pll0source').value()
            if src == 'PLL0':
                divsrc = 0
            elif src == 'PLL1':
                divsrc = 1
            ...
            outpin = 4
            divsrc = 2
        self.cdce906setoutput(outpin, divsrc, slewrate=self.parent.findParam('pll%dslew' % outnum).value(),
                              enabled=self.parent.findParam('pll%denabled' % outnum).value())
