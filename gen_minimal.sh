...
for i in hardware/naeusb hardware/firmware capture/targets capture/api common/utils capture/scopes/cwhardware; do
    mkdir -p "$i"
    touch "$i"/__init__.py
done
...
# copy naeusb files
for i in naeusb.py fpga.py bootloader_sam3u.py pll_cdce906.py serial.py spi.py; do
...
# copy target files
for i in _base.py CW305.py CW310.py ; do
