...
/**************************************************************************/
#include "../device/MPC5676R.h" /* include peripheral declarations */
#include "../device/MPC5676R_features.h"
#include "../util/compiler_api.h"
#include "../drivers/src/IRQ/vector.c"
...
void InitINTC(void) {   
    ...
    INTC->MCR  &= ~(INTC_MCR_HVEN( coreId ));     /* Software vector mode   */
#elif defined(MPC567xK)
    INTC.BCR.R &= ~(INTC_MCR_HVEN( coreId ));     /* Software vector mode   */
#else
    INTC->MCR = 0;     /* Software vector mode   */
#endif
    ...
    if (coreId) INTC->IACKR0 = (uint32_t) 0;
    else INTC->IACKR0 = (uint32_t) 0;
#else
    /* Set INTC ISR vector table base addr. */
    INTC->IACKR0 = (uint32_t) &IntcIsrVectorTable[0];
#endif
}
