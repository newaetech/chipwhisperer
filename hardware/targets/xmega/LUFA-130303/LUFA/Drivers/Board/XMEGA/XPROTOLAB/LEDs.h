...
#define __LEDS_A3BU_XPLAINED_H__
	...
	/* Public Interface - May be used in end-application: */
		...
		/* Inline Functions: */
			...
			{
				PORTD.DIRSET    = LEDS_ALL_LEDS;
				PORTD.OUTCLR    = LEDS_ALL_LEDS;
			...
			{
				PORTD_OUTSET = LEDMask;
			}

			static inline void LEDs_TurnOffLEDs(const uint8_t LEDMask)
			{
				PORTD_OUTCLR = LEDMask;
			}

			static inline void LEDs_SetAllLEDs(const uint8_t LEDMask)
			{
				PORTD_OUT = (PORTD.OUT & ~LEDS_ALL_LEDS) | LEDMask;
			}

			static inline void LEDs_ChangeLEDs(const uint8_t LEDMask, const uint8_t ActiveMask)
			{
				PORTD_OUT = (PORTD.OUT & ~LEDMask) | ActiveMask;
			}

			static inline void LEDs_ToggleLEDs(const uint8_t LEDMask)
			{
				PORTD_OUTTGL = LEDMask;
			}

			static inline uint8_t LEDs_GetLEDs(void) ATTR_WARN_UNUSED_RESULT;
			static inline uint8_t LEDs_GetLEDs(void)
			{
				return (PORTD_OUT & LEDS_ALL_LEDS);
			}
		#endif
